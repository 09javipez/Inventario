...
<head>
    <meta charset="UTF-8">
    <title>Detalle de la transferencia</title>
    <style>
...
<body>
    ...
    <div class="title">Detalle de la transferencia #{{ $model->serie }}-{{ str_pad($model->correlative, 4, '0', STR_PAD_LEFT) }}</div>

    <div>
        <strong>Fecha:</strong> {{ \Carbon\Carbon::parse($model->date)->format('d/m/Y') }}<br>
        <strong>Alamcén de origen:</strong> {{ $model->originWarehouse->name ?? '—' }}<br>
        <strong>Almacén de destino:</strong> {{ $model->destinationWarehouse->name ?? '—' }}<br>
